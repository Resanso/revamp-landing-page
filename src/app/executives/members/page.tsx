...
export default async function ExecutivesPage({ searchParams }: Props) {
  const caller = await getCaller();
  const params = await searchParams;

  // Ambil semua tahun dari database
  ...
  // Ambil member dari database
  const { members } = activeYear
    ? await caller.executives.getAll({
        year: activeYear,
    ...
    <main className="mx-auto min-h-screen w-full max-w-6xl px-4 py-14 md:px-8">
      ...
      {years.length === 0 ? (
        <p className="mt-8 text-sm text-black/60">
          Belum ada data executive members.
        </p>
      ) : (
        <>
              ...
              >
                <div className="relative aspect-square bg-[#f5f5f5]">
                  ...
                  {member.image ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center text-black/20 text-sm">
                      No Photo
                    </div>
                  )}
                </div>
